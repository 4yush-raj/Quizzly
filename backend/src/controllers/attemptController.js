...
const submitAttempt = async (req, res) => {
  try {
    ...
    // Fetch quiz and its questions from PostgreSQL DB
    const quiz = await prisma.quiz.findUnique({ where: { id: quizIdInt } });
    if (!quiz) {
      return res.status(404).json({ error: 'Quiz not found.' });
    }

    const questions = await prisma.question.findMany({ where: { quizId: quizIdInt } });
    ...
    // Resolve database user ID dynamically by email or token ID
    let dbUserId = userId;
    if (userEmail) {
...
const getAttemptById = async (req, res) => {
  try {
    ...
    if (role !== 'ADMIN' && att.userId !== userId && (att.user && att.user.email.toLowerCase() !== (userEmail || '').toLowerCase())) {
